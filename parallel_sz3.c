...
//        SZ_Finalize();
	...
	for(int i=0; i<num_vars; i++){
        ...
        unsigned char *bytesOut = SZ::sz_compress<float>(dataIn, r3, r2, r1, rel_bound[i], compressed_size[i]);
//		unsigned char *bytesOut = SZ_compress_args(SZ_FLOAT, dataIn, &compressed_size[i], REL, 0, rel_bound[i], 0, r5, r4, r3, r2, r1);
    ...
    for(int i=0; i<num_vars; i++){
        ...
        if(world_rank == 0) start = MPI_Wtime();
        float *dataOut = SZ::decompress(conf,compressed_output_pos, compressed_size[i]);
